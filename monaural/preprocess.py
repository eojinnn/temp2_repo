...
class ResnetConformer_2026(nn.Module):
    def __init__(self, in_channel, in_dim, out_dim, fs=24000, sig_len=1023, params=None):
        super().__init__()
        self.fs = fs
    ...
    def forward(self, x):
        ...
        t0 = sync_time()                                                                                #time
        # (B, C, T, W) -> (B * C * T, 1, W)
        x_sinc_in = x.reshape(B * C * T, 1, W)
        print(x_sinc_in.shape)
        sinc_feat = self.encoder(x_sinc_in)  
        t1 = sync_time()                                                                                #time
        print(sinc_feat.shape)
        _, F_dim, _ = sinc_feat.shape
        # (B*C*T, F) -> (B, C, T, F)
        sinc_feat = sinc_feat.reshape(B, C, T, F_dim)
